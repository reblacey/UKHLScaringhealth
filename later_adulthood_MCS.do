...
tokenize "`c(alpha)'"
gen sfm_1ob_om=a_sf12mcs_dv if wavefirstobv_om==1
forval waveno = 2/10 {
  replace sfm_1ob_om=``waveno''_sf12mcs_dv if wavefirstobv_om==`waveno'
}
recode sfm_1ob_om (min/-1=.)


tokenize "`c(alpha)'"
gen sfp_1ob_om=a_sf12pcs_dv if wavefirstobv_om==1
forval waveno = 2/10 {
  replace sfp_1ob_om=``waveno''_sf12pcs_dv if wavefirstobv_om==`waveno'
}
recode sfp_1ob_om (min/-1=.)
...
parenclass_h urban_1ob_om children_1ob_om workcom_om  higestedu sfm_1ob_om if omidcare==1



kmatch ps omidcare age_1ob_om i.ethnicity i.incomqu_1obom i.class_1ob_om i.workcom_om i.marriage_1ob_om i.parenclass_h i.urban_1ob_om i.children_1ob_om i.higestedu i.wavesobs  (sfm_1ob_om sfp_1ob_om), ematch(sex) nn(2) idgenerate(controlid) idvar(pidp)generate(kpscore)
...
save "C:\Users\baowen\OneDrive - University College London\EUROCARE\data\1to2matchedexact_om_sfm.dta"
...
keep pidp *age_dv  *sf12mcs_dv  ageonset_all omidcare age_1ob_om agemax agemin kpscore sex hhid_1obya meanhour_omg placecare_om
...
combomarginsplot file1_om_yn file2_om_yn, labels ("carers" "non carers") savefile(combined_sfm_yn, replace)

mplotoffset using combined_sfm_yn, xline(7,lpattern(shortdash)lcolor(gray)) xline(8,lpattern(shortdash)lcolor(gray)) title("Age 50-64") xtitle ("Years centred on care onset year") ytitle (SF12-MCS) xlabel(0 "-8" 1 "-7" 2 "-6" 3 "-5" 4 "-4" 5 "-3" 6 "-2" 7 "-1" 8 "0" 9 "1" 10 "2" 11 "3" 12 "4" 13 "5" 14 "6" 15 "7" 16 "8")ciopts(msize(vsmall))
...
combomarginsplot file1_om_yn_m file2_om_yn_m file1_om_yn_f file2_om_yn_f, labels ("male carers" "male non carers" "female carers" "female non carers") savefile(combined_sfm_yn_sex, replace)

mplotoffset using combined_sfm_yn_sex, xline(7,lpattern(shortdash)lcolor(gray)) xline(8,lpattern(shortdash)lcolor(gray)) title("Age 50-64") xtitle ("Years centred on care onset year") ytitle (SF12-MCS) xlabel(0 "-8" 1 "-7" 2 "-6" 3 "-5" 4 "-4" 5 "-3" 6 "-2" 7 "-1" 8 "0" 9 "1" 10 "2" 11 "3" 12 "4" 13 "5" 14 "6" 15 "7" 16 "8")ciopts(msize(vsmall))



/////////////////////////////////graphs for care intensity 
mixed sf12mcs_dv i.poyrtoset if kpscore==1 & meanhour_omg==1|| hhid_1obya:,covariance(unstructured)mle var
margins i.poyrtoset,saving(file1_om_low, replace)
mixed sf12mcs_dv i.poyrtoset if kpscore==1 & meanhour_omg==2|| hhid_1obya:,covariance(unstructured)mle var
margins i.poyrtoset,saving(file1_om_lmid, replace)
mixed sf12mcs_dv i.poyrtoset if kpscore==1 & meanhour_omg==3|| hhid_1obya:,covariance(unstructured)mle var
margins i.poyrtoset,saving(file1_om_mid, replace)
mixed sf12mcs_dv i.poyrtoset if kpscore==1 & (meanhour_omg==4|meanhour_omg==5) || hhid_1obya:,covariance(unstructured)mle var
...
combomarginsplot file1_om_low file1_om_lmid file1_om_mid file1_om_hig file1_om_ncare, labels ("<5h per week" "5-9h per week" "10-19h per week" "20+h per week" "non carers") savefile(combined_sfm_intensity, replace)

mplotoffset using combined_sfm_intensity, xline(7,lpattern(shortdash)lcolor(gray)) xline(8,lpattern(shortdash)lcolor(gray)) title("Age 50-64") xtitle ("Years centred on care onset year") ytitle (SF12-MCS) xlabel(0 "-8" 1 "-7" 2 "-6" 3 "-5" 4 "-4" 5 "-3" 6 "-2" 7 "-1" 8 "0" 9 "1" 10 "2" 11 "3" 12 "4" 13 "5" 14 "6" 15 "7" 16 "8")ciopts(msize(vsmall))


/////////////////////////////////graphs for care place

mixed sf12mcs_dv i.poyrtoset if kpscore==1 & (placecare_om==1|placecare_om==3)|| hhid_1obya:,covariance(unstructured)mle var
margins i.poyrtoset,saving(file1_om_in, replace)
mixed sf12mcs_dv i.poyrtoset if kpscore==1 & (placecare_om==2)|| hhid_1obya:,covariance(unstructured)mle var
margins i.poyrtoset,saving(file1_om_out, replace)
mixed sf12mcs_dv i.poyrtoset if kpscore==0|| hhid_1obya:,covariance(unstructured)mle var
margins i.poyrtoset,saving(file1_om_nocare, replace)

combomarginsplot file1_om_in file1_om_out file1_om_nocare, labels ("inside household care" "outside household care" "non carers") savefile(combined_sfm_place, replace)

mplotoffset using combined_sfm_place, xline(7,lpattern(shortdash)lcolor(gray)) xline(8,lpattern(shortdash)lcolor(gray)) title("Age 50-64") xtitle ("Years centred on care onset year") ytitle (SF12-MCS) xlabel(0 "-8" 1 "-7" 2 "-6" 3 "-5" 4 "-4" 5 "-3" 6 "-2" 7 "-1" 8 "0" 9 "1" 10 "2" 11 "3" 12 "4" 13 "5" 14 "6" 15 "7" 16 "8")ciopts(msize(vsmall))
...
replace careplace=2 if kpscore==1&(placecare_om==1|placecare_om==3)
replace careplace=1 if kpscore==1&placecare_om==2
